Feature: Pick Your Slot activity booking for badminton slot after membership type Yearly end to end

  Background:
    Given user login with username "97901401501" and password "123456"
    And user click a "Login" Button
    Then the user navigated to pys home screen

  Scenario Outline: User buy a membership type "Yearly"
    ...
    And user select "Yearly" Membership plan user click "Join Now" button
  ...
  limit in Yearly type membership
